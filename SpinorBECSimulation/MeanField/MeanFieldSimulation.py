...
def generate_states_coherent(N1,N0,Nm1,theta,nsamps):
    """compute barnetts initital TWA distribution of spinor wave functions"""
    #draw initial distributions
    a = np.random.normal(loc = 0, scale = 1, size = nsamps)
    b = np.random.normal(loc = 0, scale = 1, size = nsamps)
    c = np.random.normal(loc = 0, scale = 1, size = nsamps)
    d = np.random.normal(loc = 0, scale = 1, size = nsamps)
    e = np.random.normal(loc = 0, scale = 1, size = nsamps)
    f = np.random.normal(loc = 0, scale = 1, size = nsamps)

    N = N1 + N0 + Nm1
    delta = np.array([a+1j*b,c+1j*b,e+1j*f])/(2 * np.sqrt(N))
    psi0 = np.array([np.sqrt(N1/N),np.sqrt(N0/N)*np.exp(np.complex(0,-theta/2)),np.sqrt(Nm1/N)])
    psi = np.asarray([psi0 + i for i in delta.T])
    return psi

def generate_states_fock(N1,N0,Nm1,theta,s):
    ...
    #states = np.asarray([i for i in states if i[1].imag == 0])
    return states
...
generate_states = generate_states_coherent
